//-----------------------------------------------------------------------
// <copyright file="UdpAnySourceMulticastChannel.cs" company="Microsoft Corporation">
...
namespace NodeGardenLib
{
    ...
    /// <summary>
    /// The channel used for UDP communications
    /// </summary>
    public class UdpAnySourceMulticastChannel : IDisposable
    {
        ...
        /// <summary>
        /// Gets a value indicating whether this instance is joined.
        /// </summary>
        ...
        public void Open()
        {
            try
            {
                if (!this.IsJoined && !this.joinPending)
                {
                    this.joinPending = true;
                    this.Client.BeginJoinGroup(this.OpenCallback, null);
                }
            }
            catch (SocketException socketEx)
            {
                // See if we can do something when a SocketException occurs.
                this.HandleSocketException(socketEx);
            }
        }
        ...
        public void Send(string message)
        {
            try
            {
                if (this.IsJoined)
                {
                    ...
                    this.Client.BeginSendToGroup(data, 0, data.Length, this.SendToGroupCallback, null);
                }
            }
            catch (SocketException socketEx)
            {
        ...
        public void SendTo(IPEndPoint endPoint, string format, params object[] args)
        {
            try
            {
                if (this.IsJoined)
                {
                    ...
                    this.Client.BeginSendTo(data, 0, data.Length, endPoint, this.SendToCallback, null);
                }
                else
                {
                    ////DiagnosticsHelper.SafeShow("Not joined!");
                    this.HandleError("Not joined!", null);
                }
            }
            catch (SocketException socketEx)
            {
        ...
        private void OpenCallback(IAsyncResult result)
        {
            try
            {
                ...
                // We DO NOT want to receive the messages we send.
        ...
        /// <summary>
        /// Sends to group callback.
        /// </summary>
        /// <param name="result">The result.</param>
        private void SendToGroupCallback(IAsyncResult result)
        {
            try
            {
                if (!this.IsDisposed)
        ...
        private void Receive()
        {
            if (this.IsJoined)
            {
                ...
                if (!this.IsDisposed)
                {
                    this.Client.BeginReceiveFromGroup(
                        this.ReceiveBuffer,
                        0,
                        this.ReceiveBuffer.Length,
                        this.ReceiveFromGroupCallback,
                        null);
                }
            }
        }
        ...
        /// <summary>
        /// Receives from group callback.
        /// </summary>
        /// <param name="result">The result.</param>
        private void ReceiveFromGroupCallback(IAsyncResult result)
        {
            try
            {
                if (!this.IsDisposed)
                {
                    ...
                    // It can then be used to send unicast messages, targeted to that one recipient. These messages
        ...
        /// to your applications specific behaviour.
        /// </remarks>
        /// <param name="socketEx">The socket exception</param>
        private void HandleSocketException(SocketException socketEx)
        {
            if (socketEx.SocketErrorCode == SocketError.NetworkDown)
            {
                ////DiagnosticsHelper.SafeShow("A SocketExeption has occurred. Please make sure your device is on a WiFi network and the WiFi network is operational");
                this.HandleError("A SocketExeption has occurred. Please make sure your device is on a WiFi network and the WiFi network is operational", socketEx);
            }
            ...
            else if (socketEx.SocketErrorCode == SocketError.AccessDenied)
            {
                ...
                this.HandleError("An error occurred. Try Again.", socketEx);
            }
            else
            {
                // Just display the message.
                ////DiagnosticsHelper.SafeShow(socketEx.Message);
                this.HandleError(null, socketEx);
            }
        }
        ...
        {
            EventHandler<ExceptionEventArgs> handler = this.ErrorOccurred;

            if (handler != null)
            {
                handler(this, new ExceptionEventArgs(message, exception));
            }
        }
    }
}